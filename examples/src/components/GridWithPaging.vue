<template>
  <div>
    <p class="block">
      Paging can be achieved in three ways: either specify the maximum number of items per page or load the content as you go, or just display data from a server.
    </p>
    ...
    <div class="content">
      ...
      <data-grid
        title="Paging with fixed dataset"
        :model-value="rows"
    ...
    <div class="content">
      <p class="block">
        If you want to load data on demand but display everything loaded so far, just set the <span class="is-family-monospace">pagingMode</span> property to
        <span class="is-family-monospace">ShowMore</span> and create a handler for the <span class="is-family-monospace">@showMore</span> event
      </p>
      <p class="block">Please not that sorting is disabled in this case</p>
      <data-grid
        title="Using dynamic dataset"
        :model-value="showMoreRows"
        v-model:columns="cols"
        :is-sortable="true"
        :paging-mode="GridPagingMode.ShowMore"
    ...
    <div class="content">
      <p class="block">
        Using server side paging requires you to set <span class="is-family-monospace">pagingMode</span> property to <span class="is-family-monospace">Remote</span>,
        <span class="is-family-monospace">itemCount</span> property to the total item count and create a handler for the <span class="is-family-monospace">@paging</span> event
      </p>
      <p class="block">Please not that sorting is disabled in this case</p>
      <data-grid
        title="Using server side paging"
        :model-value="remoteRows"
        v-model:columns="remoteCols"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      cols: [
        new GridColumnDefinition("name", "Name", (p) => p.name).withFormat((n) => `${n.first} ${n.middle ?? ""} ${n.last}`).withSortForValues((a, b) => this.sortNames(a, b)),
      ...
      remoteCols: [
        new GridColumnDefinition("name", "Name", (p) => p.name).withFormat((n) => `${n.first} ${n.middle ?? ""} ${n.last}`).withRemoteSort(),
        new GridColumnDefinition("age", "Age", (p) => p.dateOfBirth).withFormat((d: Date) => new Date(Date.now() - d.valueOf()).getFullYear() - 1970 + " years").withRemoteSort(),
        new GridColumnDefinition("dbay", "Birthday", (p) => p.dateOfBirth).withFormat((d: Date) => d.getDate() + "/" + d.getMonth())
      ],
