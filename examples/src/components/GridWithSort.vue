<template>
  <div>
    <p class="block">
      This example shows how you can define sorting. You'll need to define sorting for each of the columns. If you don't, then you won't be able to sort on that column.
    </p>
    <p class="block">
      You can define a sorting algorithm for a column that has access to the whole model object for the row using <span class="is-family-monospace">.withSort()</span><br />
      Alternatively you can define sorting using just the column's value using <span class="is-family-monospace">.withSortForValues()</span>
    </p>
    ...
    return {
      ...
      cols: [
        new GridColumnDefinition("name", "Name", (p) => p.name)
          ...
          .withSortForValues((a, b) => {
            const r1 = a.first.localeCompare(b.first);
            return r1 == 0 ? a.last.localeCompare(b.last) : r1;
          }),
